...
import { teacherGateway } from "@/src/data/core/teachers";
...
export default function TeachersPage() {
  ...
  const load = useCallback(
    async (nextPage: number) => {
      setStatus("loading");
      const result = await teacherGateway.list({
        page: nextPage,
        pageSize: PAGE_SIZE,
  ...
  useEffect(() => {
    load(1).catch((nextError: unknown) => {
      setStatus("error");
      setError(nextError instanceof Error ? nextError.message : "加载老师列表失败");
    });
  }, [load]);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = editingId ? await teacherGateway.update(editingId, payload) : await teacherGateway.create(payload);
      if (result.kind === "forbidden") {
        setStatus("forbidden");
        return;
      }
      ...
      toast.error(nextError instanceof Error ? nextError.message : "操作失败");
    } finally {
      setSubmitting(false);
    }
  }

  const handleToggleTeacherStatus = useCallback(async (row: Teacher) => {
    try {
      const nextStatus = row.status === "在职" ? "停用" : "在职";
      const result = await teacherGateway.updateStatus(row.id, nextStatus);
      if (result.kind === "forbidden") {
        setStatus("forbidden");
        return;
      }
      ...
      toast.error(nextError instanceof Error ? nextError.message : "更新状态失败");
    }
  }, [load, page]);
      ...
      <Card>
        <CardContent className="space-y-3 p-4">
          <div className="grid gap-3 md:grid-cols-5">
            ...
            />
            <Select value={form.status} onValueChange={(value: "在职" | "停用") => setForm((prev) => ({ ...prev, status: value }))}>
              <SelectTrigger>
